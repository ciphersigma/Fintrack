        ...
        <div key={date}>
          ...
          {txs.map((tx) => {
            ...
            return (
              ...
              >
                <div className="group flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 hover:bg-indigo-50/30 transition-colors">
                  ...
                  {/* Desktop-only buttons (hidden on mobile where swipe is used) */}
                  <div className="hidden sm:flex gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => onEdit(tx)} className="p-1.5 rounded-lg text-gray-300 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" aria-label="Edit">
                      <HiPencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => onDelete(tx.id)} className="p-1.5 rounded-lg text-gray-300 hover:text-rose-600 hover:bg-rose-50 transition-colors" aria-label="Delete">
